...
def suppress_weak_runs_supported_by_memory_clusters(
    baseline_predictions: pd.DataFrame,
    gated_predictions: pd.DataFrame,
    suppressed_events: pd.DataFrame,
    scores: pd.DataFrame,
    ...
    cluster_padding_points: int,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    if suppressed_events.empty:
        return gated_predictions, suppressed_events

    indexed_events = suppressed_events.copy()
    indexed_events["start_time"] = pd.to_datetime(indexed_events["start_time"])
    indexed_events["end_time"] = pd.to_datetime(indexed_events["end_time"])
    indexed_events["event_id"] = indexed_events["prototype_id"].astype(str).str.split(":", n=1).str[0]

    threshold_by_channel = {
        channel: max(float(global_thresholds[channel_index]), EPSILON)
        for channel_index, channel in enumerate(target_channels)
    }
    timestamp_to_position = {timestamp: index for index, timestamp in enumerate(baseline_predictions.index)}
    rescued = gated_predictions.copy()
    added_rows: list[dict[str, Any]] = []

    def iter_positive_runs(values: np.ndarray, offset: int) -> list[tuple[int, int]]:
        runs: list[tuple[int, int]] = []
        run_start: int | None = None
        for relative_index, value in enumerate(values):
            if int(value) == 1:
                if run_start is None:
                    run_start = offset + relative_index
                continue
            if run_start is not None:
                runs.append((run_start, offset + relative_index - 1))
                run_start = None
        if run_start is not None:
            runs.append((run_start, offset + len(values) - 1))
        return runs

    for _, event_group in indexed_events.groupby("event_id", sort=False):
        event_group = event_group.sort_values(["start_time", "end_time"], kind="mergesort")
        active_cluster: list[dict[str, Any]] = []
        active_end_index = -1

        def flush_cluster() -> None:
            nonlocal rescued, added_rows
            if not active_cluster:
                return
            support_channels = {str(row["channel"]) for row in active_cluster}
            if len(support_channels) < min_support_channels:
                return

            cluster_start = min(int(row["start_index"]) for row in active_cluster)
            cluster_end = max(int(row["end_index"]) for row in active_cluster)
            padded_start = max(0, cluster_start - cluster_padding_points)
            padded_end = min(len(baseline_predictions.index) - 1, cluster_end + cluster_padding_points)

            prototype_rows = sorted(active_cluster, key=lambda row: float(row["score"]), reverse=True)
            prototype_id = str(prototype_rows[0]["prototype_id"])
            metric = str(prototype_rows[0]["metric"])

            for channel in target_channels:
                channel_values = rescued[channel].iloc[padded_start : padded_end + 1].to_numpy(dtype=np.uint8)
                if not channel_values.any():
                    continue

                threshold = threshold_by_channel[channel]
                for run_start, run_end in iter_positive_runs(channel_values, padded_start):
                    run_scores = scores[channel].iloc[run_start : run_end + 1]
                    if run_scores.empty:
                        continue
                    peak_ratio = float(run_scores.max()) / threshold
                    if peak_ratio > max_peak_ratio:
                        continue
                    rescued.iloc[run_start : run_end + 1, rescued.columns.get_loc(channel)] = 0
                    added_rows.append(
                        {
                            "channel": channel,
                            "start_time": baseline_predictions.index[run_start].isoformat(),
                            "end_time": baseline_predictions.index[run_end].isoformat(),
                            "prototype_id": prototype_id,
                            "score": float(max(float(row["score"]) for row in active_cluster)),
                            "metric": metric,
                        }
                    )

        for row in event_group.to_dict(orient="records"):
            start_index = timestamp_to_position.get(pd.Timestamp(row["start_time"]))
            end_index = timestamp_to_position.get(pd.Timestamp(row["end_time"]))
            if start_index is None or end_index is None:
                continue
            row["start_index"] = int(start_index)
            row["end_index"] = int(end_index)

            if not active_cluster:
                active_cluster = [row]
                active_end_index = row["end_index"]
                continue

            if row["start_index"] <= active_end_index + cluster_padding_points:
                active_cluster.append(row)
                active_end_index = max(active_end_index, row["end_index"])
                continue
    ...
    if not added_rows:
        return rescued, suppressed_events
    return rescued, pd.concat([suppressed_events, pd.DataFrame(added_rows)], ignore_index=True)
...
def run_tcn_split(
    ...
    gated_predictions, suppressed_events = suppress_weak_runs_supported_by_memory_clusters(
        baseline_predictions=baseline_predictions,
        gated_predictions=gated_predictions,
        suppressed_events=suppressed_events,
        scores=baseline_scores,
