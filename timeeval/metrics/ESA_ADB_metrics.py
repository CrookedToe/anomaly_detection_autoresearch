...
class ESAScores(Metric):
    ...
    def score(self, y_true: pd.DataFrame, y_pred: np.ndarray) -> dict:
        ...
        range_upper_candidates = [self.full_range[1].value]
        if events_pred_ns:
            range_upper_candidates.append(np.max(events_pred_ns))
        if events_gt_ns:
            range_upper_candidates.append(np.max(events_gt_ns))
        corrected_full_upper_range = max(range_upper_candidates)
        ...
        precision = float(np.mean(precision_list)) if precision_list else float("nan")
        recall = float(np.mean(recall_list)) if recall_list else float("nan")
